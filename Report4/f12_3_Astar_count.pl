...
astar( Start, Path) :- asserta(count(0)),
    expand( [], l( Start, 0/0),  9999, _, yes, RP),
        %  Assume 9999 is greater than any f-value
    reverse(RP,Path),  count(M), write('Count is: '), write(M), !.
...
/******************************************************************************/
update_count(Succ) :-
  retract(count(C)) , length(Succ,LenSucc) ,
  C1 is C + LenSucc , asserta(count(C1)),
   !.
