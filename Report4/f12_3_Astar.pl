...
astar( Start, Path) :-
    ...
    reverse(RP,Path)
	,count(M), write('Count is: '), write(M), nl	% New
	.		
...
succlist( G0, [N/C | NCs], Ts)  :-
    ...
    %h( N, H),								% Remove 
    h_functor(H_functor),					% New
   	Heuristic =.. [H_functor, N, H],		% New
	call(Heuristic),						% New
    F is G + H,
    succlist( G0, NCs, Ts1),
...
update_count(Succ) :-						% New
  retract(count(C)) , length(Succ,LenSucc), % New
  C1 is C + LenSucc , asserta(count(C1)),	% New
   !.
